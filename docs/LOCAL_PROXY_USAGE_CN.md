...
# Created: ~/.config/rausu/config.yaml  ← 编辑此文件
...
不指定 `--config` 时，Rausu 按以下优先级搜索配置文件：
...
若均未找到，将在 `${XDG_CONFIG_HOME:-~/.config}/rausu/config.yaml` 生成注释版模板，并提示编辑后重启。

### `rausu init` 选项

```bash
rausu init                         # 写入模板到 XDG 默认位置
rausu init --path ./config.yaml    # 写入到自定义路径
