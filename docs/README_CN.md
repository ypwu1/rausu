...
- ✅ **GitHub Copilot Provider** — 通过 Copilot 订阅使用 Claude 和 GPT 模型
- ✅ **ChatGPT 订阅 Provider** — 通过 ChatGPT Plus/Pro/Max 订阅使用 GPT 模型
- ✅ **OpenAI 兼容 Provider 支持** — 通过 `provider: openai` + `base_url` 接入 DeepSeek、Qwen、Ollama、GLM、Moonshot、百川、Yi、MiniMax 等任意 OpenAI 兼容 API
- ✅ **协议桥接** — Responses API ↔ Messages API 双向转换；Codex CLI 可使用 Claude 模型或任意 OpenAI 兼容服务，Claude Code 可使用 GPT 模型或任意 OpenAI 兼容服务；完整的工具调用和思维块支持
...
| `POST` | `/v1/responses` | OpenAI Responses API — 直传或 Responses→Messages 桥接 |
| `POST` | `/v1/responses/compact` | OpenAI Responses API 紧凑变体 — 透明直传 |
| `POST` | `/v1/messages` | Anthropic Messages API — 直传或 Messages→Responses 桥接 |
...
Rausu 实现了 OpenAI Responses API 与 Anthropic Messages API 之间的双向协议桥接，支持任意客户端与模型组合：
...
**桥接特性：**
- 完整工具调用支持 — `function_call` ↔ `tool_use`，自动处理参数 JSON 序列化
- 思维/推理块转换 — `reasoning` ↔ `thinking`
- 真正的 SSE 流式传输 — 基于 async-stream 的零缓冲逐事件中继；首 token 延迟与直传路径一致
- 自动检测 — 根据 provider 和模型组合自动激活桥接，无需客户端任何配置

详细协议映射说明见 [PROTOCOL_BRIDGE_PLAN_CN.md](PROTOCOL_BRIDGE_PLAN_CN.md)（[English](PROTOCOL_BRIDGE_PLAN.md)）。

## 设计文档

- [本地代理使用指南](LOCAL_PROXY_USAGE_CN.md)（[English](LOCAL_PROXY_USAGE.md)）— 连接 Codex CLI 和 Claude Code，含跨协议使用方法
- [协议桥接方案](PROTOCOL_BRIDGE_PLAN_CN.md)（[English](PROTOCOL_BRIDGE_PLAN.md)）— Responses ↔ Messages API 转换
- [GitHub Copilot Provider](GITHUB_COPILOT_PROVIDER_CN.md)（[English](GITHUB_COPILOT_PROVIDER.md)）
- [ChatGPT 订阅 Provider](CHATGPT_SUBSCRIPTION_PROVIDER_CN.md)（[English](CHATGPT_SUBSCRIPTION_PROVIDER.md)）
...
- [TLS 和 mTLS 支持](TLS_MTLS_CN.md)（[English](TLS_MTLS.md)）— 传输层加密与双向 TLS
- [工具兼容性与能力检查（草案）](TOOL_COMPATIBILITY_DRAFT_CN.md)（[English](TOOL_COMPATIBILITY_DRAFT.md)）— 工具感知透传、Provider 能力模型、禁止静默降级规则
