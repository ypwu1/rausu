...
//! transform layer, the same strategy used by the `openrouter`, `openai`, and
//! `z-ai` providers.
//!
...
impl Provider for MoonshotProvider {
    ...
    async fn chat_completions(
        ...
        debug!(url = %url, model = %req.model, "Sending non-streaming request to Moonshot");
        ...
        if !response.status().is_success() {
            ...
            error!(status = status, body = %body, "Moonshot error response");
    ...
    > {
        let url = format!("{}/chat/completions", self.base_url);
        debug!(url = %url, model = %req.model, "Sending streaming request to Moonshot");
        ...
        if !response.status().is_success() {
            let body = response.text().await.unwrap_or_default();
            error!(status = status, body = %body, "Moonshot streaming error response");
            return Err(ProviderError::ProviderResponse {
                status,
                message: body,
            });
    ...
    async fn proxy_responses(
        ...
        // Moonshot speaks the OpenAI Chat Completions format but does not natively
        // support the Responses API. Bridge through Chat Completions, the same
        // strategy used by the openrouter, openai, and z-ai providers.
        let cc_body = transform::responses_to_chat_completions_request(&body);

        let url = format!("{}/chat/completions", self.base_url);
        debug!(url = %url, "Sending Responses→CC bridged request via moonshot");
        ...
        if !status.is_success() {
            ...
            error!(status = status_code, body = %msg, "Moonshot Responses CC bridge error");
            return Err(ProviderError::ProviderResponse {
                status: status_code,
                message: msg,
            });
...
mod tests {
    ...
    fn make_provider() -> MoonshotProvider {
    ...
    fn test_provider_name() {
        assert_eq!(make_provider().name(), "moonshot");
    }
    ...
    #[test]
    fn test_models_owned_by_moonshot() {
        let p = MoonshotProvider::new(
            "key".to_string(),
            None,
        ...
        for m in &models {
            assert_eq!(m.owned_by, "moonshot");
            assert_eq!(m.object, "model");
        }
        assert_eq!(models[0].id, "moonshot-v1-8k");
        assert_eq!(models[1].id, "moonshot-v1-32k");
    }
    ...
    fn test_sse_valid_chunk_parsed() {
        ...
        assert_eq!(chunk.id, "chatcmpl-moonshot");
        assert_eq!(chunk.model, "moonshot-v1-8k");
    }
