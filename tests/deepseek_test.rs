//! Smoke tests for the DeepSeek provider integration.
//!
//! These tests exercise DeepSeek through Rausu's route handlers using stub
//! providers (no live DeepSeek API calls). They cover:
//!
...
//! - Failover from unsupported provider to DeepSeek
...
/// A stub that behaves like a working DeepSeek provider for chat completions.
struct StubDeepSeekProvider {
    model_names: Vec<String>,
}

#[async_trait]
impl Provider for StubDeepSeekProvider {
    fn name(&self) -> &str {
        "deepseek"
    }
    ...
    async fn chat_completions(
        ...
        Ok(ChatCompletionResponse {
            id: "chatcmpl-deepseek-test".to_string(),
            ...
            choices: vec![Choice {
                ...
                message: Message {
                    ...
                    content: Some(Value::String("Hello from DeepSeek!".to_string())),
                    tool_call_id: None,
                    tool_calls: None,
                    name: None,
                },
    ...
    > {
        let chunks = vec![
            Ok(ChatCompletionChunk {
                id: "chatcmpl-deepseek-stream".to_string(),
            ...
            Ok(ChatCompletionChunk {
                ...
                choices: vec![ChunkChoice {
                    ...
                    delta: Delta {
                        role: None,
                        content: Some(" from DeepSeek!".to_string()),
                        tool_calls: None,
                    },
                    finish_reason: Some("stop".to_string()),
                }],
    ...
    async fn proxy_responses(
        ...
        let http_resp = http::Response::builder()
            ...
            .body(bytes::Bytes::from(
                r#"{"id":"resp_deepseek_test","object":"response"}"#,
            ))
            .unwrap();
...
struct DeepSeekAuthErrorProvider;

#[async_trait]
impl Provider for DeepSeekAuthErrorProvider {
    fn name(&self) -> &str {
        "deepseek"
    }
...
struct DeepSeekUnsupportedProvider;

#[async_trait]
impl Provider for DeepSeekUnsupportedProvider {
    fn name(&self) -> &str {
        "anthropic"
    }
...
struct DeepSeekNoToolsProvider;

#[async_trait]
impl Provider for DeepSeekNoToolsProvider {
    fn name(&self) -> &str {
        "deepseek"
    }
...
#[tokio::test]
async fn test_deepseek_chat_non_stream() {
    let app = make_chat_app(
        vec![Box::new(StubDeepSeekProvider {
            model_names: vec!["deepseek-chat".to_string()],
        })],
        vec![registry_entry("deepseek-chat", "deepseek", "deepseek-chat")],
    );
    let resp = post_json(
        app,
        "/v1/chat/completions",
    ...
    assert_eq!(body["id"], "chatcmpl-deepseek-test");
...
async fn test_deepseek_chat_stream() {
    ...
    let resp = post_json(
        app,
        "/v1/chat/completions",
        r#"{"model": "deepseek-chat", "stream": true, "messages": [{"role": "user", "content": "Hi"}]}"#,
    )
    .await;
    assert_eq!(resp.status(), 200);
    ...
    assert!(
        text.contains("from DeepSeek!"),
        "expected 'from DeepSeek!' in SSE stream"
    );
    assert!(text.contains("[DONE]"), "expected [DONE] sentinel");
}

#[tokio::test]
async fn test_deepseek_responses_api() {
    let app = make_chat_app(
        vec![Box::new(StubDeepSeekProvider {
            model_names: vec!["deepseek-chat".to_string()],
        })],
    ...
    let resp = post_json(
        app,
        "/v1/responses",
        r#"{"model": "deepseek-chat", "input": [{"role": "user", "content": "Hi"}]}"#,
    )
    .await;
    assert_eq!(resp.status(), 200);

    let body = body_json(resp).await;
    assert_eq!(body["id"], "resp_deepseek_test");
}

#[tokio::test]
async fn test_deepseek_invalid_auth() {
    let app = make_chat_app(
        vec![Box::new(DeepSeekAuthErrorProvider)],
        vec![registry_entry("deepseek-chat", "deepseek", "deepseek-chat")],
    );
    let resp = post_json(
        app,
...
#[tokio::test]
async fn test_deepseek_tools_passthrough() {
    let app = make_chat_app(
        vec![Box::new(StubDeepSeekProvider {
            model_names: vec!["deepseek-chat".to_string()],
        })],
...
#[tokio::test]
async fn test_deepseek_response_format_passthrough() {
    let app = make_chat_app(
        vec![Box::new(StubDeepSeekProvider {
            model_names: vec!["deepseek-chat".to_string()],
        })],
...
#[tokio::test]
async fn test_deepseek_unsupported_capability_failover() {
    // First provider doesn't support chat completions; DeepSeek does.
    let app = make_chat_app(
        vec![
            Box::new(DeepSeekUnsupportedProvider),
            Box::new(StubDeepSeekProvider {
...
#[tokio::test]
async fn test_deepseek_unsupported_capability_no_fallback() {
    let app = make_chat_app(
        vec![Box::new(DeepSeekUnsupportedProvider)],
...
#[tokio::test]
async fn test_deepseek_response_format_unsupported_returns_422() {
    let app = make_chat_app(
        vec![Box::new(DeepSeekNoToolsProvider)],
        vec![registry_entry("deepseek-chat", "deepseek", "deepseek-chat")],
    );
...
#[test]
fn test_capability_declaration_deepseek_stub() {
    let p = StubDeepSeekProvider {
...
#[tokio::test]
async fn test_deepseek_build_providers_registration() {
    ...
    // Verify that deepseek is a valid provider string in the config schema.
    assert_eq!(config.models[0].providers[0].provider, "deepseek");
    assert_eq!(config.models[0].providers[0].model, "deepseek-chat");
}
